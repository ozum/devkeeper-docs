...
export default async function typedocMd(this: Command) {
  ...
  try {
    ...
    await execa(c('typedoc'), ['--platform', 'vuepress', '--plugin', 'typedoc-plugin-example-tag,typedoc-plugin-markdown', '--excludeExternals', '--excludePrivate', '--excludeProtected', '--theme', 'markdown', '--readme', 'none', '--mode', 'file', '--out', 'api-docs-md'], {stdio: 'inherit'})
